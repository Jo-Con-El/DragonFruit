...
import { runGradientFlowRouter } from './GradientFlowRouter';
...
export function calculateSmartPlacementV2(
    ...
    const gfResult = runGradientFlowRouter(sdf, socketPos, rootTopZ, {
        clearance,
        gridEnabled: settings.grid.enabled,
        spacingMm: settings.grid.spacingMm,
        diskHeight,
    ...
    if (!gfResult.reached) {
        ...
        return {
            ...standard,
            error: gfResult.error === 'STAGNATED' ? 'STAGNATED' : 'COLLISION_WITH_MODEL',
        };
    }

    const pathJoints = gfResult.path.slice(1, -1);
    const pathEnd = gfResult.path[gfResult.path.length - 1];
    const unsnappedBottomPos = { x: pathEnd.x, y: pathEnd.y, z: 0 };

    let bestBase = {
        basePos: gfResult.basePos,
        rootTopTarget: { x: gfResult.basePos.x, y: gfResult.basePos.y, z: rootTopZ },
        inboundLateralMm: 0,
        snapDistance: 0,
        nodeKey: gfResult.snappedNodeKey,
    };
