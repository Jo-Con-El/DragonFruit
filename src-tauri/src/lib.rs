// lib.rs — shared Tauri app setup for desktop and iOS/iPadOS.
// Desktop entry point: main.rs calls run().
// Mobile entry point: tauri::mobile_entry_point generates the Swift FFI shim.
...
async fn local_backup_pick_directory(current_path: Option<String>) -> Result<String, String> {
    #[cfg(mobile)]
    return Err("Directory picker not available on this platform".to_string());
...
async fn save_print_file(args: SavePrintFileArgs) -> Result<String, String> {
    #[cfg(mobile)]
    return Err("File save dialog not available on this platform".to_string());
...
async fn pick_open_files(args: PickOpenFilesArgs) -> Result<Vec<PickedOpenFile>, String> {
    #[cfg(mobile)]
    return Err("File open dialog not available on this platform".to_string());
...
async fn reveal_in_file_manager(path: String) -> Result<(), String> {
    #[cfg(mobile)]
    return Err("Reveal in file manager not available on this platform".to_string());
...
async fn launch_external_process(exe_path: String, file_arg: String) -> Result<(), String> {
    #[cfg(mobile)]
    return Err("Launching external processes not available on this platform".to_string());
...
async fn open_log_file(app: DragonFruitAppHandle) -> Result<(), String> {
    #[cfg(mobile)]
    return Err("Opening log file in editor not available on this platform".to_string());
...
pub fn run() {
    ...
    let builder = builder.setup(|app| {
        ...
        // On mobile (iOS/iPadOS), Tauri provides a single WebView automatically —
        // the splashscreen + deferred main-window pattern is desktop-only.
        #[cfg(not(mobile))]
        ...
        }); // end #[cfg(not(mobile))] spawn

        Ok(())
    });

    // Single-instance plugin: disabled on CEF (wry collision) and on mobile (irrelevant).
    #[cfg(all(not(feature = "tauri-cef"), not(mobile)))]
    ...
    // download + install across all platforms. Desktop-only (App Store handles iOS updates).
    #[cfg(not(mobile))]
    let builder = builder.plugin(tauri_plugin_updater::Builder::new().build());
    // Process plugin — needed for app relaunch after update installs. Desktop-only.
    #[cfg(not(mobile))]
